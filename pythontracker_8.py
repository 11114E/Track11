...
def index():
    ...
    for file_name in os.listdir(CSV_DIRECTORY):
        if file_name.endswith('.csv'):
            ...
            try:
                ...
                if 'Balance' in df.columns and df['Balance'].dtype == 'object':
    ...
    if data_frames:
        ...
        all_peer_ids = sorted(set(latest_balances['Peer ID']) | set(last_1440_quil_per_day.index) | set(last_1440_quil_per_hour.index))

        # Reindex all Series to have the same index
        latest_balances = latest_balances.set_index('Peer ID').reindex(all_peer_ids)
        last_1440_quil_per_day = last_1440_quil_per_day.reindex(all_peer_ids)
        last_1440_quil_per_hour = last_1440_quil_per_hour.reindex(all_peer_ids)

        # Fill NaN values with 0
        latest_balances = latest_balances.fillna(0)
        last_1440_quil_per_day = last_1440_quil_per_day.fillna(0)
        last_1440_quil_per_hour = last_1440_quil_per_hour.fillna(0)

        # Add 24-Hour Quil Per Hour and 24-Hour Quil Per Day to latest_balances
        latest_balances['24-Hour Quil Per Day'] = last_1440_quil_per_day
        latest_balances['24-Hour Quil Per Hour'] = last_1440_quil_per_hour
